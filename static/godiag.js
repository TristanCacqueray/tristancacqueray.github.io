 ...
 *
 **/
if (typeof window.GoDiagram === 'undefined') {
  window.GoDiagram = class {
  linkmap = {}
  ...
  drawIntersection(
    ...
    if (!type.includes("B")) {
      svgElem = `<line x1="${x}" y1="${y +
        this.radius}" x2="${x}" y2="${y}" stroke="${color}" />
  `
      intersectionElements += svgElem
    }
    if (!type.includes("L")) {
  ...
  drawCoordinates(
    color,
    coordClass,
    SVGTextSize // Returns one or more svg elements with the Goban coordinates
  ) {
    var coordChars =
...
// Highlight.js integration (C) by
// Tristan Cacqueray 2024 <tristan.cacqueray@gmail.com>

// console.log("Installing godiag")
hljs.configure({ignoreUnescapedHTML: true});
hljs.registerLanguage("baduk", () => ({keywords: []}));
hljs.addPlugin({
  'before:highlight': obj => {
    if (obj.language === "baduk" && obj.code[0] != '\n') {
      // console.log("HERE", obj);
      const goban = new GoDiagram(obj.code);
      const svg = goban.createSVG();
      obj.result = { value: "<p>" + goban.getTitle() + "</p>" + svg.xml };
    }
  }
});
